...
const notifyUpdate = (res: VoidCheckUpdateRespose & { message: string }, notifService: INotificationService, updateService: IUpdateService): INotificationHandle => {
	const message = res?.message || 'This is a very old version of Nap Editor, please download the latest version! [Nap Editor](https://voideditor.com/download-beta)!'
...
const notifyErrChecking = (notifService: INotificationService): INotificationHandle => {
	const message = `AI Error: There was an error checking for updates. If this persists, please get in touch or reinstall Nap Editor [here](https://voideditor.com/download-beta)!`
	const notifController = notifService.notify({
		severity: Severity.Info,
		message: message,
		sticky: true,
...
const performVoidCheck = async (
	...
	const metricsTag = explicit ? 'Manual' : 'Auto'
		...
		metricsService.capture(`AI Update ${metricsTag}: Checking...`, {})
	const res = await voidUpdateService.check(explicit)
	if (!res) {
		const notifController = notifyErrChecking(notifService);
			metricsService.capture(`AI Update ${metricsTag}: Error`, { res })
		return notifController
	}
	else {
		if (res.message) {
			const notifController = notifyUpdate(res, notifService, updateService)
			metricsService.capture(`AI Update ${metricsTag}: Yes`, { res })
			return notifController
		}
		else {
			metricsService.capture(`AI Update ${metricsTag}: No`, { res })
			return null
		}
	}
}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('voidCheckUpdate', 'AI: Check for Updates'),
		});
	}
